...
class Upsell_Bundle_Order_Bump
{
    ...
    private function is_bump_in_cart($bump_product_id, $main_product_id)
    {
        ...
        foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
            if (
                $cart_item['product_id'] == $bump_product_id &&
    ...
    public function render_order_bumps()
    {
        ...
        echo '<div class="upsell-bundle-checkout-bumps order-bump-section">';
        echo '<h4>' . esc_html__('Tambahan Sekalian?', 'woo-express-checkout') . '</h4>';
...
?>
            <label class="order-bump upsell-bundle-bump-card <?php echo $checked ? 'bump-checked' : ''; ?>">
                <div class="upsell-bump-checkbox-wrapper">
                    <input type="checkbox"
                        ...
                        <div class="bump-price">
                            <?php if ($bump_price < $original_price) : ?>
                                <del><?php echo wc_price($original_price); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                                        ?></del>
                            <?php endif; ?>
                            <span><?php echo wc_price($bump_price); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                                    ?></span>
                        </div>
                    </div>
                </div>

                <div class="bump-action" aria-hidden="true">
                    <span class="add-text">+ <?php esc_html_e('Tambah', 'woo-express-checkout'); ?></span>
                    <span class="added-text">✓ <?php esc_html_e('Ditambahkan', 'woo-express-checkout'); ?></span>
                </div>
            </label>
<?php
        }
